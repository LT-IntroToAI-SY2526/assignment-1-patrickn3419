...
def every_other(lst: List[T]) -> List[T]:
    ...
    #Java way
    # result = []
    # for i in range(len(lst)): #i stands for index, len: Length, lst: list
    #     if i % 2 == 0:
    #         result.append(lst[i])
    # return result
    return lst[::2]
...
def median(lst: List[int]) -> float:
    ...
    """
    if len(lst) % 2 == 1:
        return lst[len(lst) // 2] #Double division = integer division
    else:
...
def duck_duck_goose(lst: List[str]) -> List[str]:
    ...
    pos = 0
    while len(lst) > 2:
        pos = (pos + 2) % len(lst) #Move forward 2 steps while wrapping around
...
if __name__ == "__main__":
    ...
    assert absolute(34) == 34, "absolute of 34 failed"
    ...
    assert sum_list([1, 1, 1, 0]) == 3, "sum_list of [1, 1, 1] failed"
    assert mean([1, 2, 3, 4, 5]) == 3, "mean of [1,2,3,4,5] failed"
    assert mean([2, 2, 2, 2, 2, 2, 2]) == 2, "mean of [2,2,2,2,2,2,2] failed"
